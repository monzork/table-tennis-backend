...
// TestBuildPdfBracketRounds_DuplicateFirstRoundMatchDoesNotDuplicatePlayer
// is a regression test mirroring
// domain/bracket.TestBuildBracketRounds_DuplicateFirstRoundMatchDoesNotDuplicatePlayer:
// a stale/duplicate quarterfinal match row for the same player (bad
// DivisionID tagging leaving behind an extra record) must not render that
// player as the winner of two different sibling PDF bracket matches.
func TestBuildPdfBracketRounds_DuplicateFirstRoundMatchDoesNotDuplicatePlayer(t *testing.T) {
	...
	rounds := buildPdfBracketRounds(ev, "d1", players)

	seen := map[string]int{}
	for _, r := range rounds {
		for _, m := range r.Matches {
			for _, slot := range []*pdfMatchSlot{m.Player1, m.Player2} {
...
// TestBuildPdfBracketRounds_PreviewSemifinalRespectsQuarterAdjacency mirrors
// domain/bracket.TestBuildBracketRounds_PreviewSemifinalRespectsQuarterAdjacency:
// once all four quarterfinals are real/finished but no semifinal match
// exists yet, the PDF's semifinal preview must pair the two quarters that
// share the same half of the draw, not whatever order the QF match rows
// happen to come back from the DB in.
func TestBuildPdfBracketRounds_PreviewSemifinalRespectsQuarterAdjacency(t *testing.T) {
	p := make(map[string]*player.Player, 8)
	for i := 1; i <= 8; i++ {
		id := "p" + string(rune('0'+i))
		p[id] = &player.Player{ID: id, FirstName: id}
	...
	// getSeedingArrangement(8) = [1,8,5,4,3,6,7,2]: canonical quarterfinals
	// are (seed1 vs seed8) and (seed5 vs seed4) in one half, (seed3 vs
	// seed6) and (seed7 vs seed2) in the other. Rows inserted out of that
	// order (QF3 first) to reproduce the bug.
	win := []event.MatchSet{{Number: 1, ScoreA: 11, ScoreB: 5}}
	ev := &event.Event{
		Matches: []event.Match{
