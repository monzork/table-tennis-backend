...
func (uc *RegisterPlayerUseCase) Execute(ctx context.Context, firstName, lastName string, birthdate, gender, country, whatsAppNumber string, singlesElo, doublesElo int16) (*playerDomain.Player, error) {
	bd, err := time.Parse("2006-01-02", birthdate)
	if err != nil {
		return nil, err
	}

	p, err := playerDomain.NewPlayer(firstName, lastName, bd, gender, country)
	if err != nil {
		return nil, err
	}
