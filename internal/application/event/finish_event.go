	...
	if !t.SkipElo {
		...
		for _, m := range t.Matches {
			...
			for _, p := range m.TeamA {
				...
				// not an evolving in-event value. SinglesElo/DoublesElo are
				// used here purely as this loop's scratch/staging fields --
				// match.CalculateAndApplyElo and the forfeit-reversal logic
				// below operate on them unchanged; final persistence (below)
				// is what actually routes the result to the right age-
				// category Elo pool via EloFor/UpdateEloFor.
		...
		// tournamentDomain.PlacementResults. Persisted as a durable
		// per-event record (see SavePlacementResults) so it's never lost
		// once a later tournament finishes -- best-effort: a failure here
		// doesn't undo the event having finished.
