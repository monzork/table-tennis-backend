...
// to self-register into a event that has RegistrationOpen == true.
...
// GetOpenTournaments returns all events where RegistrationOpen == true.
func (uc *SelfRegisterUseCase) GetOpenTournaments(ctx context.Context) ([]*tournamentDomain.Event, error) {
	all, err := uc.tournamentRepo.GetAll(ctx)
	if err != nil {
		return nil, err
	}
	var open []*tournamentDomain.Event
	for _, t := range all {
...
// Execute attempts to register an existing player into the specified event.
...
func (uc *SelfRegisterUseCase) Execute(
	...
	gender string,
	nationalID string,
) (*tournamentDomain.Event, string, error) {
	...
	if !t.RegistrationOpen || t.Status == "finished" {
		return nil, "", errors.New("event is not open for registration")
	}
	...
	for _, p := range t.Participants {
		if p.ID == matched.ID {
			return nil, "", errors.New("player is already registered in this event")
		}
	}
