...
func (h *TournamentHandler) Detail(c *fiber.Ctx) error {
	...
	// Build player→group map from event.Groups
...
func (h *TournamentHandler) PublicTVDashboard(c *fiber.Ctx) error {
	...
	tmap, _ := c.Locals("T").(map[string]string)
	vm := BuildTournamentViewModel(res.event, res.divisions, tmap)
	vm.IsPublic = true
...
func (h *TournamentHandler) getOccupiedTables(ctx context.Context, t *tournamentDomain.Event) []int {
