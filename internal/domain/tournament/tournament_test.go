...
func TestNewTournament_Valid(t *testing.T) {
	...
	tourn, err := tournament.NewTournament("t1", "Test Tourn", "singles", "elimination", "men", start, end, nil, 2, participants, false)
	if err != nil {
...
func TestNewTournament_InvalidDates(t *testing.T) {
	...
	_, err := tournament.NewTournament("t1", "Test Tourn", "singles", "elimination", "open", start, end, nil, 2, nil, false)
	if err != tournament.ErrInvalidDates {
...
func TestNewTournament_CategoryValidation(t *testing.T) {
	...
	participants := []*player.Player{
		{ID: "p1", Gender: "F"}, // Female in a men's tournament
	}

	_, err := tournament.NewTournament("t1", "Test Tourn", "singles", "elimination", "men", start, end, nil, 2, participants, false)
	if err == nil {
...
func TestTournament_GetEffectiveStageRule(t *testing.T) {
	tourn := &tournament.Tournament{
		StageRules: []tournament.StageRule{
			{Stage: "final", BestOf: 7, PointsToWin: 11, PointsMargin: 2},
		},
		DivisionRules: []tournament.DivisionRule{
