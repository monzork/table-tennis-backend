...
func main() {
	...
	if isPostgres {
		...
		var hasCol int
		_ = bunDB.NewRaw(`SELECT COUNT(*) FROM pragma_table_info('event_division_rules') WHERE name = 'stage'`).Scan(ctx, &hasCol)

		if hasCol == 0 {
			_, err = bunDB.NewRaw(`ALTER TABLE event_division_rules ADD COLUMN stage VARCHAR(50) DEFAULT 'group'`).Exec(ctx)
			if err != nil {
	...
	log.Println("Migration 032 complete: Added stage column to event_division_rules")
}
