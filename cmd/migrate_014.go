...
func main() {
	...
	_, err = sqldb.Exec(`ALTER TABLE group_participants ADD COLUMN position INTEGER NOT NULL DEFAULT 0;`)
	if err != nil {
		log.Printf("Query error (might already exist): %v", err)
	} else {
		log.Printf("Successfully added position column to group_participants.")
	}
}
