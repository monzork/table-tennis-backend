...
func main() {
	...
	// Add pin to matches
	_, err = sqldb.Exec(`ALTER TABLE matches ADD COLUMN pin TEXT DEFAULT NULL;`)
	if err != nil {
		log.Printf("Warning adding pin column to matches: %v (might already exist)", err)
	} else {
		log.Println("Added pin column to matches.")
	}
}
