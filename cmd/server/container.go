...
func NewContainer(store *session.Store, cfg Config) *Container {
	...
	finishTournamentUC := event.NewFinishTournamentUseCase(eventRepo, matchRepo, playerRepo, divisionRepo)
	recalculateTournamentEloUC := event.NewRecalculateTournamentEloUseCase(eventRepo, matchRepo, playerRepo, divisionRepo)
