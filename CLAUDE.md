...
- Migrations are plain numbered SQL files in `cmd/migrations/`, **not applied automatically** — each needs a matching `cmd/migrate_0NN.go` one-off script (`//go:build ignore`, run via `go run cmd/migrate_0NN.go`), excluded from the coverage gate. `cmd/migrations/postgresql_init.sql` is a manually-maintained full-schema baseline for fresh installs — keep it in sync with the latest migration when adding one.
